...
export default function App() {
    ...
    React.useEffect(() => {
        MitreDataService.getTacticSigma(params.id, params.page, 10).then( function (response) {
    ...
    function DisplaySigmaRules (currentItems) {
        if (currentItems.length === 0){
            return (
                <div>
                    <h5>No rules have been added that map to this tactic.</h5>
                </div>
            )
        }
        return (
    ...
    function getNextPage(pageNum) {
        MitreDataService.getTacticSigma(params.id, pageNum, 10).then((response) => {
            navigate(`/mitre/countermeasure/sigma/tactic/${params.id}/${pageNum}`);
            setRules(response.data['items']);
            setTotalItems(response.data['total']);
        }).catch(function (error) {
            alert(error);
