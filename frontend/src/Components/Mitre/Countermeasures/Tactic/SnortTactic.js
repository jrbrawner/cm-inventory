...
export default function App() {
    ...
    React.useEffect(() => {
        MitreDataService.getTacticSnort(params.id, params.page, 10).then( function (response) {
    ...
    function DisplaySnortRules (currentItems) {
        ...
        if (currentItems.length === 0){
            return (
                <div>
                    <h5>No snort rules have been added that map to this tactic.</h5>
                </div>
            )
        }
    ...
    function getNextPage(pageNum) {
        MitreDataService.getTacticSnort(params.id, pageNum, 10).then((response) => {
            navigate(`/mitre/countermeasure/snort/tactic/${params.id}/${pageNum}`);
            setRules(response.data['items']);
            setTotalItems(response.data['total']);
        }).catch(function (error) {
            alert(error);
