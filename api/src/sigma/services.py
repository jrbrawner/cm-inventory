                ...
                if keywords[0] == "attack":
                    if len(keywords) == 3:
                        ...
                        if re.match(pattern_technique, keywords[1].capitalize()):
                            technique = db.query(Technique).get(
                                keywords[1].capitalize()
                            )
                        ...
                        if tactic_name is not None:
                            tactic = (
                                db.query(Tactic)
                                .filter(Tactic.name == tactic_name)
                                .first()
                            )
                            rule_db.tactics.append(tactic)
        db.add(rule_db)
        if rule_db.title is not None:
            sigma_rule_list.append(
                {"msg": f"{rule_db.title} added.", "variant": "success"}
            )
        else:
            sigma_rule_list.append(
                {"msg": "Rule with no title added.", "variant": "success"}
            )
    db.commit()
        ...
        for tag in tags:
            ...
            if keywords[0] == "attack":
                if len(keywords) == 3:
                    ...
                    if tactic_name is not None:
                        tactic = (
                            db.query(Tactic).filter(Tactic.name == tactic_name).first()
                        )
...
def get_sigma_rule_description(db: Session, value: str) -> Page[SigmaRule]:
    return paginate(
        db.query(SigmaRule).filter(SigmaRule.description.like(f"%{value}%"))
    )
...
def get_sigma_rules_tactics(db: Session, value: str) -> Page[SigmaRule]:
    ...
    if re.match(pattern_tactic, value.upper()):
        return paginate(
            db.query(SigmaRule).filter(SigmaRule.tactics.any(id=value.upper()))
        )
...
def get_sigma_rules_techniques(db: Session, value: str) -> Page[SigmaRule]:
    ...
    if re.match(pattern_technique, value.upper()):
        return paginate(
            db.query(SigmaRule).filter(SigmaRule.techniques.any(id=value.upper()))
        )
    ...
    if technique_id is not None:
        return paginate(
            db.query(SigmaRule).filter(SigmaRule.techniques.any(id=technique_id))
        )
...
def get_sigma_rules_subtechniques(db: Session, value: str) -> Page[SigmaRule]:
    ...
    if re.match(pattern_subtechnique, value.upper()):
        return paginate(
            db.query(SigmaRule).filter(SigmaRule.subtechniques.any(id=value.upper()))
        )
    ...
    if subtechnique_id is not None:
        return paginate(
            db.query(SigmaRule).filter(SigmaRule.subtechniques.any(id=subtechnique_id))
        )
