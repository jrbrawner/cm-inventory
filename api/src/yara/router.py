...
def get_yara_rule(field: YaraRuleFieldSearch, value:str,  db: Session = Depends(get_db)):
    if field is YaraRuleFieldSearch.Name:
        ...
        if yara_rules is None:
            raise HTTPException(404, 'No rules found searching for that field with that value.')
        return yara_rules
...
def update_yara_rule(id: int, rule_text: str, db: Session = Depends(get_db)) -> YaraSchema:
