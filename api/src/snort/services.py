    ...
    for rule in rules:
        ...
        if result is None:
                        ...
                        if key == "rem":
                            ...
                            for i in mitre:
                                ...
                                if opts[0].strip() == "subtechnique":
                                    subtechnique_db = db.query(Subtechnique).get(
                                        opts[1]
                                    )
            ...
            if db_rule.msg is None:
                snort_rule_list.append(
                    {
                        "msg": f"Rule with no name added to database.",
                        "variant": "success",
                    }
                )
            else:
            ...
            snort_rule_list.append(
                {"msg": f"Duplicate rule detected. {result.msg}", "variant": "danger"}
            )
...
def get_snort_rule_body_options(db: Session, value: str) -> list[SnortRule]:
    return paginate(
        db.query(SnortRule).filter(SnortRule.body_options.like(f"%{value}%"))
    )
...
def get_snort_rules_tactics(db: Session, value: str) -> list[SnortRule]:
    ...
    if re.match(pattern_tactic, value.upper()):
        return paginate(
            db.query(SnortRule).filter(SnortRule.tactics.any(id=value.upper()))
        )
...
def get_snort_rules_techniques(db: Session, value: str) -> list[SnortRule]:
    ...
    if re.match(pattern_technique, value.upper()):
        return paginate(
            db.query(SnortRule).filter(SnortRule.techniques.any(id=value.upper()))
        )
    ...
    if technique_id is not None:
        return paginate(
            db.query(SnortRule).filter(SnortRule.techniques.any(id=technique_id))
        )
...
def get_snort_rules_subtechniques(db: Session, value: str) -> list[SnortRule]:
    ...
    if re.match(pattern_subtechnique, value.upper()):
        return paginate(
            db.query(SnortRule).filter(SnortRule.subtechniques.any(id=value.upper()))
        )
    ...
    if subtechnique_id is not None:
        return paginate(
            db.query(SnortRule).filter(SnortRule.subtechniques.any(id=subtechnique_id))
        )
...
def update_snort_rule(db: Session, id: int, rule_text: str) -> SnortRule:
    ...
    parser = SnortParser()
    rule_list = parser.parse_rules(rule_text)
