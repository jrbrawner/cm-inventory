...
def check_status() -> dict:
    ...
    try:
        status = requests.get(f"http://{snort_url}/configuration")
...
def create_configuration(config_name: str, HOME_NET: str, EXTERNAL_NET: str) -> str:
    ...
    configuration = requests.post(url=f"{snort_url}/configuration", params=params)
...
def test_rule(db: Session, id: int, rule_string: str) -> str:
    ...
    if rule_string is not None:
        ...
        result = requests.get(f"{snort_url}/test-rule", params=params)
...
def convert_rule(db: Session, id: int, rule_string: str) -> str:
    ...
    if rule_string is not None:
        params = {"rule_string": rule_string}
        result = requests.post(f"{snort_url}/snort2lua/convert-rule", params=params)
        return result.content
    rule = db.query(SnortRule).get(id)
    params = {"rule_string": rule.raw_text}
    result = requests.post(f"{snort_url}/snort2lua/convert-rule", params=params)
...
def convert_all_rules(db: Session):
    ...
    for rule in rules:
        ...
        if "Rule loaded successfully." not in str(test_result):
            rule_list.append(
                {"id": rule.id, "result": test_result, "rule_string": rule.raw_text}
            )
    ...
    for entry in rule_list:
        ...
        if result is None:
            updated_list.append(
                {
                    "id": entry["id"],
                    "updated_string": "Cant be updated, check snort.rej",
                }
            )
    et = time.time()
...
def read_pcap(pcap_file: UploadFile) -> str:
    ...
    result = requests.post(f"{snort_url}/read-pcap", files=files)
    return result.content
...
def read_pcap_detailed(pcap_file: UploadFile, show_raw_packet_data: bool) -> str:
    ...
    result = requests.post(
        f"{snort_url}/read-pcap-detailed", files=files, params=params
    )
...
def analyze_pcap_id(db: Session, id: int, pcap_file: UploadFile) -> str:
    ...
    contents = pcap_file.file.read()
    files = {
        "pcap_file": ("pcap_file", contents),
        "rules_file": ("rules_file", temp_file_rules),
    }
    f.close()

    result = requests.post(f"{snort_url}/analyze-pcap", files=files)
...
def analyze_pcap_detailed(db: Session, id: int, pcap_file: UploadFile) -> str:
    ...
    result = requests.post(f"{snort_url}/analyze-pcap-detailed", files=files)
