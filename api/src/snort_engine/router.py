...
@router.get(
    "/api/snort-engine/status", response_class=JSONResponse, tags=["snort-engine"]
)
def check_status():
...
@router.get(
    "/api/snort-engine/configuration",
...
@router.get(
    "/api/snort-engine/configuration/{name}",
    response_class=PlainTextResponse,
    tags=["snort-engine"],
)
...
def create_new_configuration(
    config_name: str, HOME_NET: str, EXTERNAL_NET: str = "!$HOME_NET"
):
...
@router.get(
    "/api/snort-engine/test-rule",
    response_class=PlainTextResponse,
    tags=["snort-engine"],
)
def test_rule_compilation(
    id: int | None = None, rule_string: str = None, db: Session = Depends(get_db)
):
...
@router.post(
    "/api/snort-engine/read-pcap-detailed",
    response_class=PlainTextResponse,
    tags=["snort-engine"],
)
...
@router.post(
    "/api/snort-engine/analyze-pcap/input",
    response_class=PlainTextResponse,
    tags=["snort-engine"],
)
def analyze_pcap_input(
    pcap_file: UploadFile,
    files: list[UploadFile] | None = None,
    rule_text: str = Form(None),
):
    contents = ""
    if files is not None:
        ...
        if result is None:
            raise HTTPException(
                400, "Error in analyzing pcap with provided rules files."
            )
        return result
...
@router.post(
    "/api/snort-engine/analyze-pcap/{id}",
    response_class=PlainTextResponse,
    tags=["snort-engine"],
)
...
@router.post(
    "/api/snort-engine/analyze-pcap-detailed",
    response_class=PlainTextResponse,
    tags=["snort-engine"],
)
def analyze_pcap_detailed(
    pcap_file: UploadFile, id: int, db: Session = Depends(get_db)
):
